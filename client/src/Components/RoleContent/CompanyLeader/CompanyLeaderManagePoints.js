...
import { Box, Heading, Text, Flex, Button, Stack, IconButton } from '@chakra-ui/react';
...
const CompanyLeaderManagePoints = () => {
  ...
  const handleAddTransactionPoints = (index) => {
    ...
    console.log(`Added transaction points for ${updatedTransactionPoints[index].location}`);
  };
  ...
  const handleSubtractTransactionPoints = (index) => {
    ...
    console.log(`Subtracted transaction points for ${updatedTransactionPoints[index].location}`);
  };
  ...
  const handleAddGatheringPoints = (index) => {
    ...
    console.log(`Added gathering points for ${updatedGatheringPoints[index].location}`);
  };
  ...
  const handleSubtractGatheringPoints = (index) => {
    ...
    console.log(`Subtracted gathering points for ${updatedGatheringPoints[index].location}`);
  };
    ...
    <Box p={4}>
      ...
      <Box mb={6}>
        ...
        {transactionPoints.map((point, index) => (
          <Flex key={index} alignItems="center" justifyContent="space-between" py={2}>
